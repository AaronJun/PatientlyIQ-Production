<script lang="ts">
  ...
  function handleAreaClick(event) {
    const { area, data, view } = event.detail;
  ...
  function drawVisualization() {
    ...
    therapeuticAreas.forEach(area => {
      ...
      .on("click", () => handleAreaLabelClick(area));
// Add the click handler function

function handleAreaClick(area: string) {
  const taData = constellationData.filter(d => d.name === area);
  if (taData.length > 0) {
    dispatch('areaClick', {
      area,
      data: taData[0],
  ...
  function handleAreaLabelClick(area: string) {
    const taData = constellationData.filter(d => d.name === area);
    if (taData.length > 0) {
      selectedAreaData = taData[0];
      drawerView = 'ta';
      drawerColor = therapeuticAreaColors[area as keyof typeof therapeuticAreaColors] || '#000000';
      isDrawerOpen = true;
    }
  }
