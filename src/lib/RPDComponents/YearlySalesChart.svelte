<script lang="ts">
  ...
  function handleClick(transactionData: any) {
    if (transactionData.originalData) {
      ...
      const colorKey = transactionData.originalData.TherapeuticArea1 || 
  ...
  function createChart() {
    ...
    const salesData = constellationData
      ...
      .map(d => {
        ...
        return {
          ...
          price: price,
          drugName: d.Candidate || '',  // Using Candidate as drug name
          buyer: d.Purchaser || 'Unknown',
          seller: d.Company || 'Unknown',  // Using Company as seller
          therapeuticArea: d.TherapeuticArea1 || 'Unknown', // Using TherapeuticArea1
          indication: d.Indication || '', // Using Indication
    ...
    const maxValue = d3.max(stackedData, d => d.totalValue) || 0;
