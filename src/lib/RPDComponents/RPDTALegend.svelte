...
<div class={`absolute p-4 pl-8 pb-8 bg-white dark:bg-gray-800 z-10 
  ...
  <div class="grid grid-cols-1 pb-1 sm:grid-cols-2 gap-1 sm:gap-3">
    {#each Object.entries(therapeuticAreaColors) as [area, color]}
      <div 
        class="flex items-center gap-2 min-w-[140px] cursor-pointer hover:bg-orange-300 dark:hover:bg-gray-700 p-1 rounded transition-colors duration-150"
        on:mouseenter={() => handleMouseEnter(area)}
        on:mouseleave={handleMouseLeave}
      >
        <div 
          class="w-2 h-3 ring-1 ring-sky-900 dark:border-gray-200 rounded-full flex-shrink-0"
