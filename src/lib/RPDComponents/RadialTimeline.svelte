<script lang="ts">
  ...
  $: height = Math.max(containerHeight || 1920, 1200);
  $: petalScale = Math.min(width, height) / 1475; // Add scaling factor
  ...
  $: innerRadius = Math.min(width, height) * 0.0125;
  $: outerRadius = Math.min(width, height) / 1.275 - Math.max(margin.top, margin.right, margin.bottom, margin.left);
  ...
  function createPetalCluster(year: string, entries: ConstellationEntry[], position: { x: number, y: number, angle: number }, animate: boolean = false) {
    ...
    function drawYearCircle() {
        ...
        yearLabels = svg.append("g")
            ...
            .enter()
            .append("text")
            .attr("x", d => Math.sin(xScale(d)! + xScale.bandwidth() / 2) * (yearCircleRadius + width * 0.057))
            .attr("y", d => -Math.cos(xScale(d)! + xScale.bandwidth() / 2) * (yearCircleRadius + width * 0.0267))
            .attr("text-anchor", "middle")
            .attr("alignment-baseline", "middle")
            .text(d => d)
            .attr("font-size", `0.6725rem`)
...
.timeline-container {
  width: auto;
  height: 80vh; /* Match the grid template row height */
  display: flex;
