...
<script lang="ts">
    ...
    export let height = 180; // Increased height to accommodate all labels above
    ...
    function processTimelineData(data) {
      ...
      Object.entries(milestones).forEach(([label, date]) => {
        if (date && date.trim() !== "") {
          // Get color for the event - either from eventColors or from the therapeutic area
          const color = eventColors[label] || getStageColor(label).fill;
    ...
    function drawTimeline() {
      ...
      const timeBuffer = (lastDate - firstDate) * 0.05; // 5% buffer
      ...
      events.forEach((event, i) => {
        ...
        // Always position labels above the timeline
