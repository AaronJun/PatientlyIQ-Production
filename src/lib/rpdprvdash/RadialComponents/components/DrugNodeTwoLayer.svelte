<script lang="ts">
    ...
    export let nodeRadius: number | undefined = undefined; // Optional custom radius
    ...
    export function createDrugNode() {
        ...
        // Use custom radius if provided, otherwise use default from sizeConfig
        const radius = nodeRadius !== undefined ? nodeRadius : sizeConfig.drugNodeRadius;
        // Adjust stroke width proportionally to node size
        const strokeWidth = nodeRadius !== undefined 
            ? sizeConfig.drugNodeStrokeWidth * (nodeRadius / sizeConfig.drugNodeRadius)
        ...
        drugGroup.on("focus", function(event: FocusEvent) {
            ...
            // Calculate appropriate highlight radius based on whether a custom radius is provided
            const highlightedRadius = nodeRadius !== undefined 
                ? nodeRadius * (sizeConfig.highlightedNodeRadius / sizeConfig.drugNodeRadius)
                : sizeConfig.highlightedNodeRadius;
            ...
            if (sizeConfig.useAnimations) {
                ...
                circle.attr("r", highlightedRadius);
            }
