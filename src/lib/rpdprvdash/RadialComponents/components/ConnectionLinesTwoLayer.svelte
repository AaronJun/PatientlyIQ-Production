<script lang="ts">
     ...
     */
    export function createDrugConnection(startX: number, startY: number, drugX: number, drugY: number, companyName: string, drugId: string, isSubConnection: boolean = false) {
        if (!linesGroup) return;
        
        // Use simplified connections in all year view or when specified
        const strokeDasharray = sizeConfig.simplifiedConnections ? (isSubConnection ? "0.5,3" : "1,2") : "none";
        // Thinner lines for sub-connections
        const strokeWidth = isSubConnection ? sizeConfig.connectionStrokeWidth * 0.8 : sizeConfig.connectionStrokeWidth;
        // Slightly lower opacity for sub-connections
        const opacity = isSubConnection ? sizeConfig.connectionOpacity * 0.85 : sizeConfig.connectionOpacity;
        
        const path = linesGroup.append("path")
            .attr("class", "drug-connection")
            .attr("data-company", companyName)
