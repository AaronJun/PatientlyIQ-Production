...
<script lang="ts">
    ...
    function handleStageClick(stage: string) {
        if (selectedStage === stage) {
            ...
            onStageHover(data.filter(entry => entry.developmentStage === stage));
        }
    }
    ...
    let width = 1200;
    let height = 1000;
    const margin = { top: 0, right: 40, bottom: 200, left: 120 };
    const innerWidth = width - margin.left - margin.right;
    const innerHeight = (height - margin.bottom) * 1.125;
    ...
    function highlightStage(stage: string) {
        ...
        // Enhance stage's circles and desaturate others
        g.selectAll("circle")
            .transition()
            .duration(200)
            .style("opacity", d => d.developmentStage === stage ? 1 : 0.2)
            .attr("r", d => d.developmentStage === stage ? circleRadius * 1.2 : circleRadius)
            .style("stroke", d => d.developmentStage === stage ? "#000" : "none")
            .style("stroke-width", d => d.developmentStage === stage ? 1 : 0);
    }
    ...
    function highlightRow(company: string) {
        ...
        // Enhance company's circles and desaturate others
        g.selectAll("circle")
            .transition()
            .duration(200)
    ...
    function createVisualization() {
        ...
        // Add X axis with hover behavior
        ...
        xAxis.selectAll(".tick")
            ...
            .on("mouseenter", (event, d) => {
                if (!selectedCompany && !selectedStage) {
                    highlightStage(d);
                    onStageHover(data.filter(entry => entry.developmentStage === d));
                }
            })
            .on("mouseleave", () => {
                if (!selectedCompany && !selectedStage) {
            ...
            .on("click", (event, d) => handleStageClick(d));
            
        xAxis.selectAll("text")
            .attr("transform", "rotate(-45)")
            .style("text-anchor", "end");

        // Add Y axis with hover behavior
        const yAxis = g.append("g")
            .attr("class", "y-axis")
            .attr("height", innerHeight)
            .call(d3.axisLeft(yScale));

   // Add hover and click behavior to y-axis labels
   yAxis.selectAll(".tick")
            ...
            .on("click", (event, d) => {
                const companyEntries = data.filter(entry => entry.Company === d);
                onShowCompanyDetail({
                    Company: d,
                    entries: companyEntries,
                    color: '#37587e',
