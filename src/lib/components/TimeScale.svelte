<script lang="ts">
	...
	// Get study phase based on day
	function getPhaseFromDay(day: number): string {
		if (day <= 0) return 'Screening'; // Include day 0 (randomization) in screening
		if (day <= 56) return 'Treatment'; // 8 weeks
		return 'Follow-up';
	}
</script>
...
<div class="time-scale" style="width: {timelineWidth}px;">
	...
	<div class="phase-indicators">
		...
		{#if maxDay > 56}
			<div class="phase-indicator followup" style="left: {((56 - minDay) / (maxDay - minDay)) * (1 - 0.2) * timelineWidth + 0.1 * timelineWidth}px; width: {((maxDay - 56) / (maxDay - minDay)) * (1 - 0.2) * timelineWidth}px;">
