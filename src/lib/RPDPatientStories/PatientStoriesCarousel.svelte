...
{#if isVisible && patient}
  <div 
    class="fixed inset-0 bg-gray-900/90 backdrop-blur-sm flex items-center justify-center z-50 py-8 px-8 overflow-hidden"
    on:click={closeModal}
            ...
            {#each CARD_TYPES as type, i}
              ...
              >
                <div class="px-4 py-2 rounded-full transition-all duration-200 {
                  ...
                  i < current ? 'text-orange-600 hover:bg-orange-50' : 'text-gray-400 hover:text-gray-600'
                }">
                  {type}
                </div>
              ...
              <div class="p-8">
                {#if current === 0}
                  <div class="grid lg:grid-cols-[400px,1fr] gap-8">
                    <div class="space-y-6">
                      <div class="aspect-square w-full rounded-2xl overflow-hidden shadow-lg">
                        <img 
                  ...
                  <div class="grid lg:grid-cols-[1fr,2fr] gap-8">
                    <div class="space-y-6">
                      ...
                      <div class="flex flex-wrap gap-3">
                        {#each [patient.age, patient.disease, patient.persona] as tag}
                        <span class="px-2 py-1.5 bg-slate-100 rounded-full text-xs font-medium text-gray-600">
                        {tag}
                          </span>
                        {/each}
                      </div>
                      <img 
                        src={patient.photoUrl} 
                        alt={patient.name}
                        class="w-full h-44 max-w-sm rounded-xl object-cover shadow-lg"
                      />
                      <div class="text-sm text-orange-600 font-semibold">
                        {patient.context}
                      </div>
                      <blockquote class="text-lg text-slate-800">
                        "{patient.quote}"
                      </blockquote>
                      {#if patient.sentiment}
                      <div>
                    ...
                    <div class="space-y-8" bind:clientWidth={containerWidth} bind:clientHeight={containerHeight}>
                      {#if containerWidth && containerHeight}
                        ...
                        {/if}
                        {#if patient.topics}
                          <div class="bg-gray-50 rounded-2xl p-2 shadow-sm">
                            <TopicBarChart
                              data={patient.topics}
                              width={400}
                              height={200}
